view: device_model_dau_distribution {
  derived_table: {
    sql:
    select overall.*,
          RANK() OVER (partition by event_date ORDER BY dau DESC) AS dau_rank
    from
     ( select a.device_model,
             a.event_date,
      ...
      where
            ...
            and {% condition dau_country %} country {% endcondition %}
            and {% condition reported_age_bucket_filter %} reported_age_bucket {% endcondition %}
...
group by 1,2
  ...
  filter: reported_age_bucket_filter {
    type: string
  }
