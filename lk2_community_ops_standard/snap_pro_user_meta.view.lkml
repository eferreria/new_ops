...
view: snap_pro_user_meta {
  ...
  derived_table: {
    sql:
        ...
        creation_country,
        creator_tier__tier,
        creator_tier__subtier,
        creator_tier__is_real_creator,
     FROM `search-analytics-156901.search_analysis_persistent.public_user_20*`
    WHERE _TABLE_SUFFIX = (SELECT max(REGEXP_EXTRACT(table_id, r'_20(.*)'))  FROM `search-analytics-156901.search_analysis_persistent.__TABLES__` WHERE table_id LIKE 'public_user_20%')
         ;;
  }
  ...
  dimension: creator_tier__tier {
    type: string
  }

  dimension: creator_tier__subtier {
    type: string
  }

  dimension: creator_tier__is_real_creator {
    type: yesno
